...
/* If we have 2 classes, parent and child class, in that case if we have a try block and inside that there is a possibility of both the exceptions,
so we will catch both the exceptions, we will have a catch each for parent and child class. First we specify catch for child class and after that catch for 
parent class as if we define exception of parent class first it is capable of handline and catching exception of child class as well. */
